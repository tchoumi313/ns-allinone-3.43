 ...
 */
#include "ns3/arpmec-neighbor.h"
#include "ns3/arpmec-packet.h"
#include "ns3/arpmec-rqueue.h"
#include "ns3/arpmec-rtable.h"
...
{
    TypeHeaderTest()
        : TestCase("ARPMEC TypeHeader")
    {
    }

    void DoRun() override
    {
        TypeHeader h(ARPMECTYPE_RREQ);
        NS_TEST_EXPECT_MSG_EQ(h.IsValid(), true, "Default header is valid");
        NS_TEST_EXPECT_MSG_EQ(h.Get(), ARPMECTYPE_RREQ, "Default header is RREQ");

        Ptr<Packet> p = Create<Packet>();
        p->AddHeader(h);
        TypeHeader h2(ARPMECTYPE_RREP);
 ...
 * \ingroup arpmec-test
 *
 * \brief Unit test for ARPMEC routing table entry
 */
...
struct ArpmecRqueueTest : public TestCase
{
    ArpmecRqueueTest()
        : TestCase("Rqueue"),
          q(64, Seconds(30))
...
ArpmecRqueueTest::DoRun()
{
    ...
    h.SetDestination(Ipv4Address("1.2.3.4"));
    h.SetSource(Ipv4Address("4.3.2.1"));
    Ipv4RoutingProtocol::UnicastForwardCallback ucb = MakeCallback(&ArpmecRqueueTest::Unicast, this);
    Ipv4RoutingProtocol::ErrorCallback ecb = MakeCallback(&ArpmecRqueueTest::Error, this);
    ...
    Simulator::Schedule(q.GetQueueTimeout() + Seconds(1), &ArpmecRqueueTest::CheckTimeout, this);
...
void
ArpmecRqueueTest::CheckSizeLimit()
{
 ...
 * \brief Unit test for ARPMEC routing table entry
 */
struct ArpmecRtableEntryTest : public TestCase
{
    ArpmecRtableEntryTest()
 ...
 * \ingroup arpmec-test
 *
 * \brief Unit test for ARPMEC routing table
 */
struct ArpmecRtableTest : public TestCase
{
    ArpmecRtableTest()
        : TestCase("Rtable")
    {
 ...
 * \ingroup arpmec-test
 *
 * \brief ARPMEC test suite
 */
class ArpmecTestSuite : public TestSuite
{
  public:
    ArpmecTestSuite()
        : TestSuite("routing-arpmec", Type::UNIT)
    {
        ...
        AddTestCase(new ArpmecRqueueTest, TestCase::Duration::QUICK);
        AddTestCase(new ArpmecRtableEntryTest, TestCase::Duration::QUICK);
        AddTestCase(new ArpmecRtableTest, TestCase::Duration::QUICK);
    }
} g_arpmecTestSuite; ///< the test suite
