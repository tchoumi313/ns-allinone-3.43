...
Ad Hoc On-Demand Distance Vector (ARPMEC)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand
Distance Vector (ARPMEC) protocol. The implementation is based on
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 ARPMEC model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the ARPMEC-UU implementation by Erik Nordström of
Uppsala University.

Model Description
*****************

The source code for the ARPMEC model lives in the directory `src/arpmec`.

Design
++++++

Class ``ns3::arpmec::RoutingProtocol`` implements all functionality of
...
forwarding.  The first one, ``ns3::arpmec::RouteOutput``, is used for
locally originated packets, and the second one, ``ns3::arpmec::RouteInput``,
...
this functionality are attributes of ``ns3::arpmec::RoutingProtocol``.
...
ARPMEC discovers routes on demand.  Therefore, the ARPMEC model buffers all
packets while a route request packet (RREQ) is disseminated.
A packet queue is implemented in arpmec-rqueue.cc. A smart pointer to
...
* This ARPMEC implementation can detect the presence of unidirectional
  ...
  This heuristic is taken from ARPMEC-UU implementation and can be disabled.
...
the assertion from the ARPMEC RFC that ARPMEC works over UDP.  This model uses
